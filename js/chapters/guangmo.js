...
export default function playGuangmo(container) {
    window.currentActiveChapter = 7;
    container.classList.add('active'); 

    // 🌟【核心修复】向主调度器返回原生 Promise，将其死死卡住，不放行绝不允许第八开启动！
    return new Promise(async (resolveNextChapter) => {
        ...
        async function triggerNewSnowBurialFlow() {
            ...
            // 🌟 清场并释放 Promise 放行条！此时主调度器才会收到信号执行第八开！
